sap.ui.define([
  ...
  "com/deloitte/mdg/purchase/pricing/initiator/model/keyCombinations"

], function (Controller, Fragment, JSONModel, MessageToast, Filter, FilterOperator, KeyCombinations) {
  "use strict";

  return Controller.extend("com.deloitte.mdg.purchase.pricing.initiator.controller.Submissison", {
    ...
    _loadRequestFromCAP: function (reqId) {
      ...
      const oContext = oModel.bindContext(
        `/PurchasePricingRequests(requestId='${reqId}')`,
        null,
    ...
    onAddNew: function () {
      ...
      if (!this._dialog_createConditionRecord) {
        Fragment.load({
          id: oView.getId(),
          name: "com.deloitte.mdg.purchase.pricing.initiator.fragment.CreateConditionRecord",
          controller: this
    ...
    dialog_createConditionRecord_submit: async function () {
      ...
      if (!this._dialog_keyCombination) {
        this._dialog_keyCombination = await Fragment.load({
          id: oView.getId(),
          name: "com.deloitte.mdg.purchase.pricing.initiator.fragment.KeyCombination",
          controller: this
        });
    ...
    _openF4Dialog: function (title) {
      ...
      if (!this._F4Dialog) {
        Fragment.load({
          ...
          name: "com.deloitte.mdg.purchase.pricing.initiator.fragment.F4Dialog",
          controller: this
        }).then((oDialog) => {
    ...
    async _patchRequest(payload, requestId) {
      ...
      const response = await fetch(
        `${base}PurchasePricingRequests(requestId='${requestId}')`,
        {
          method: "PATCH",
          headers: {
    ...
    onDraftPress: async function () {
      try {
        ...
        if (oDraft.requestId) {
          ...
          const listBinding = oModel.bindList("/PurchasePricingRequests");
      ...
      try {
        ...
        const wfResponse = await fetch(
          this._getWorkflowBaseURL() + "/workflow-instances",
          {
            ...
            body: JSON.stringify({
              definitionId: "com.deloitte.mdg.purchase.pricing.purchasepricingapproval",
              context: { ReqId: reqId }
            })
          }
        );
